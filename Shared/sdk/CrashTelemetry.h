...
namespace CrashTelemetry
{
    ...
    inline std::string BuildAllocationTelemetryNote() noexcept
    {
        ...
        auto appendEntryLine = [&](const Context& entry, std::size_t index, bool includeIndexLabel)
        {
            ...
            if (ageMs >= 0)
            {
                ...
                {
                    _snprintf_s(line, sizeof(line), _TRUNCATE, "  #%zu requested=%zu bytes, tag=%s, detail=%s, object=0x%p, thread=%lu, age=%llims\n", index,
                                sizeToReport, resourceTag, detailText, entry.resourcePointer, static_cast<unsigned long>(threadId), ageMs);
                }
                else
                {
                    _snprintf_s(line, sizeof(line), _TRUNCATE, "  requested=%zu bytes, tag=%s, detail=%s, object=0x%p, thread=%lu, age=%llims\n", sizeToReport,
                                resourceTag, detailText, entry.resourcePointer, static_cast<unsigned long>(threadId), ageMs);
                }
            }
            else
            {
                if (includeIndexLabel)
                {
                    _snprintf_s(line, sizeof(line), _TRUNCATE, "  #%zu requested=%zu bytes, tag=%s, detail=%s, object=0x%p, thread=%lu\n", index, sizeToReport,
                                resourceTag, detailText, entry.resourcePointer, static_cast<unsigned long>(threadId));
                }
                else
                {
                    _snprintf_s(line, sizeof(line), _TRUNCATE, "  requested=%zu bytes, tag=%s, detail=%s, object=0x%p, thread=%lu\n", sizeToReport, resourceTag,
                                detailText, entry.resourcePointer, static_cast<unsigned long>(threadId));
                }
            }
        ...
        {
            char buffer[160]{};
            _snprintf_s(buffer, sizeof(buffer), _TRUNCATE, "Allocation telemetry: requested=%zu bytes (no tagged context available)\n", recordedSize);
            note += buffer;
        }
        else
        {
    ...
    {
    public:
        Scope(std::size_t sizeBytes, const void* resourcePtr, const char* resourceTag, const char* detail) noexcept
    ...
    inline void SetAllocationContext(std::size_t, const void*, const char*, const char*) noexcept
    {
    }
