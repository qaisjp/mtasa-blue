...
bool JpegDecode(const void* pData, uint uiDataSize, CBuffer* pOutBuffer, uint& uiOutWidth, uint& uiOutHeight, std::string* pOutError)
{
    ...
    {
        if (pOutError)
            *pOutError = "Invalid JPEG dimensions: " + std::to_string(cinfo.image_width) + "x" + std::to_string(cinfo.image_height) + " (max " +
                         std::to_string(JPEG_MAX_DIMENSION) + ")";
        jpeg_destroy_decompress(&cinfo);
        return false;
    }
    ...
    {
        if (pOutError)
            *pOutError = "JPEG dimensions too large for buffer: " + std::to_string(uiWidth) + "x" + std::to_string(uiHeight) + " requires " +
                         std::to_string(uiRequiredSize) + " bytes";
        jpeg_destroy_decompress(&cinfo);
        return false;
    }
    ...
    while (cinfo.output_scanline < cinfo.output_height)
    {
        ...
        {
            if (pOutError)
                *pOutError = "Failed to read JPEG scanline " + std::to_string(cinfo.output_scanline) + " of " + std::to_string(cinfo.output_height) +
                             " (truncated image?)";
            jpeg_destroy_decompress(&cinfo);
            return false;
        }
...
bool JpegEncode(uint uiWidth, uint uiHeight, uint uiQuality, const void* pData, uint uiDataSize, CBuffer& outBuffer, std::string* pOutError)
{
    ...
    if (uiWidth > JPEG_MAX_DIMENSION || uiHeight > JPEG_MAX_DIMENSION)
    {
        if (pOutError)
            *pOutError =
                "JPEG dimensions too large: " + std::to_string(uiWidth) + "x" + std::to_string(uiHeight) + " (max " + std::to_string(JPEG_MAX_DIMENSION) + ")";
        return false;
    }
    ...
    if (uiDataSize != static_cast<uint>(uiExpectedSize))
    {
        if (pOutError)
            *pOutError =
                "Input buffer size mismatch: expected " + std::to_string(static_cast<uint>(uiExpectedSize)) + " bytes, got " + std::to_string(uiDataSize);
        return false;
    }
